...
class TestSettings:
    ...
    def test_translation_config_structure(self):
        ...
        # 检查必需的配置项
        required_keys = ["enabled", "openrouter", "default_settings", "fallback"]
    ...
    def test_boolean_settings_consistency(self):
        ...
        assert isinstance(
            TRANSLATION_CONFIG["default_settings"]["bilingual_format"], bool
        )
        assert isinstance(
            TRANSLATION_CONFIG["default_settings"]["preserve_emojis"], bool
        )
        assert isinstance(PAPER_ANALYSIS_CONFIG["enable_analysis"], bool)
        assert isinstance(PAPER_ANALYSIS_CONFIG["enable_content_collection"], bool)
